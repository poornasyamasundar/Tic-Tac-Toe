...
from opponent import getOpponentMove, createOpponent
from agent import getAgentMove, createPolicy

pygame.init()


board_layout = 4                #This is the variable which judges tha layout of the board
screen_height = board_layout*100
screen_width = board_layout*100
cols = board_layout*board_layout
rows = 3**(board_layout*board_layout)
line_width = 6
...
#create opponent
createOpponent(board_layout)

#create policy
createPolicy(board_layout)
...
while run:
        ...
        if game_over == False:
            if player == 1:
                move = getAgentMove(markers, board_layout)              #get X's movement from the agent which is selected according to the policy
                markers[move[0]][move[1]] = player
                player *= -1
                check_game_over()
            elif player == -1:
                move = getOpponentMove(markers, board_layout)           #get O's movement from the opponent which is decided based on the probability table.
                markers[move[0]][move[1]] = player
                player *= -1
                check_game_over()
